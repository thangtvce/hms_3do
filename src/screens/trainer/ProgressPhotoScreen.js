...
const ProgressPhotoScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('API Request URL:', `/api/progressphoto?${new URLSearchParams(queryParams).toString()}`);
        const photoResponse = await trainerService.getProgressPhotos(queryParams);
        console.log('API Response for ProgressPhotos:', photoResponse);
        if (photoResponse.statusCode === 200 && photoResponse.data) {
          ...
          console.log('Fetch Photo Error Details:', { statusCode: photoResponse.statusCode, message: errorMessage });
        ...
        console.log('API Response for Comparisons:', comparisonResponse);
        if (comparisonResponse.statusCode === 200 && comparisonResponse.data) {
          ...
          console.log('Fetch Comparison Error Details:', { statusCode: comparisonResponse.statusCode, message: errorMessage });
  ...
  const createComparison = async () => {
    ...
    try {
      ...
      console.log('Create Comparison Error:', error);
  ...
  const handleUploadPhoto = async () => {
    ...
    if (!pickerResult.cancelled) {
      ...
      try {
        ...
        console.log('Upload Request Data:', { comparisonId, userId: user.userId });
        ...
        if (response.statusCode === 201) {
          ...
          console.log('Upload Error Details:', { statusCode: response.statusCode, message: errorMessage });
